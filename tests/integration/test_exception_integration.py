"""
Integration tests demonstrating SPI exception usage in protocol implementations.

This module provides comprehensive integration-style tests that show developers
how to properly use SPI exceptions in real protocol implementation scenarios.
These tests serve as both validation and documentation for exception patterns.

Key Integration Patterns Demonstrated:
    1. Handler lifecycle exceptions (initialization, execution, shutdown)
    2. Registry lookup and registration errors
    3. Protocol state validation errors
    4. Error propagation through protocol layers
    5. Exception context for debugging
    6. Proper exception handling and recovery

Usage Guide:
    These tests demonstrate the expected behavior when implementing protocols
    defined in omnibase_spi. When creating protocol implementations in
    omnibase_infra or other packages, follow these patterns for consistent
...
class TestHandlerLifecycleExceptions:
    """
    Integration tests for handler lifecycle exception handling.
...
class TestRegistryExceptions:
    """
    Integration tests for registry exception handling.
...
class TestExceptionPropagation:
    """
    Integration tests for exception propagation through protocol layers.
...
class TestIdempotencyStoreExceptions:
    """
    Integration tests for idempotency store exception handling.
...
class TestProjectorExceptions:
    """
    Integration tests for projector exception handling.
...
class TestContractCompilerExceptions:
    """
    Integration tests for contract compiler exception handling.
...
class TestExceptionRecoveryPatterns:
    """
    Integration tests demonstrating exception recovery patterns.

    These tests show how to implement retry, fallback, and recovery
    logic using SPI exceptions.
    """
