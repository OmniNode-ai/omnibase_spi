...
"""The release tag must be pushed with an identity `release.yml` can hear (OMN-18658).

`release.yml`'s only automatic trigger is `push: tags: ['v[0-9]*...']`. A push made
with the job's default workflow token delivers no push event, so a tag pushed that
way is not merely mis-attributed -- it is unconsumable, and it cannot be re-pushed
without deleting it first. That is why `v0.23.4` here and `omnibase_core` `v0.47.17`
both had to be released by hand on 2026-09-17.

An App-token push DOES start push-driven CI in this org. The proof is this
repository's own run ``35134173847`` (`event: push`, `actor:
onexbot-occ-writer[bot]`, `conclusion: success`), which corrected the earlier
org-wide belief that App tokens were suppressed the same way. That belief was a
credential confound, not a platform behaviour: a default ``actions/checkout``
persists a basic-auth ``http.<origin>.extraheader`` carrying the workflow token, and
that header overrides any credential written into the remote URL.
...
drops either half fails here rather than passing review unnoticed.
"""
...
def test_the_tagger_is_a_real_job_that_pushes_a_tag() -> None:
    ...
    assert steps, (
        ...
        "reusable workflow. The tagger was inlined under OMN-18658 because the "
        "shared reusable is pinned at @main in a repository whose main is "
        "release-synced, so a fix there is not live for the next cut."
    )
    text = _step_text()
...
def test_no_workflow_delegates_tagging_to_the_pinned_reusable() -> None:
    ...
    assert not offenders, (
        ...
        "start and the cut would need a hand dispatch again (OMN-18658)."
    )
