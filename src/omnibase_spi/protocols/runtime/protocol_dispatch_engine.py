...
that route event envelopes to appropriate handlers based on topic. It is the
provider-interface contract satisfied by both the runtime
``MessageDispatchEngine`` (``omnibase_infra``) and the node-owned
``MixinNodeDispatch`` selection seam (``omnibase_core.runtime``), so the two
implementations are interchangeable behind this protocol (OMN-12549, epic
OMN-12525).
...
Architecture:
    ...
    enabling the production ``MessageDispatchEngine``, the node-owned
    ``MixinNodeDispatch``, and test mocks to be used interchangeably.

Layering:
    This protocol lives in ``omnibase_spi`` (SPI layer) and references only the
    core result model ``omnibase_core.models.dispatch.model_dispatch_result``.
    It was relocated here from ``omnibase_infra.protocols`` in OMN-12549 so the
    dispatch contract no longer depends on the infra runtime.
...
Related:
    ...
    - OMN-12525 / OMN-12549: dispatch-chain unification; protocol relocated to spi
    - EventBusSubcontractWiring: Uses this protocol for dispatch engine injection
    - MessageDispatchEngine / MixinNodeDispatch: implementations of this protocol

.. versionadded:: 0.2.5
.. versionchanged:: 0.24.0
    OMN-12549: relocated from ``omnibase_infra.protocols`` to
    ``omnibase_spi.protocols.runtime``; result import retargeted to
