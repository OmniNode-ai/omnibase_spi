...
class ValidationError:
    ...
    def __str__(self) -> str:
        ...
        return f"[{self.severity.upper()}] {self.error_type}: {self.message}{context_str}"
...
class ProtocolValidator:
    ...
    def validate_implementation(
        ...
        """
        protocol_name = protocol.__name__ if hasattr(protocol, '__name__') else str(protocol)
        ...
        if hasattr(protocol, '__runtime_checkable__') and protocol.__runtime_checkable__:
    ...
    def _validate_runtime_checkable(self, protocol: Any, result: ValidationResult) -> None:
            ...
            for attr_name, expected_type in protocol_hints.items():
                if attr_name not in impl_hints:
                    result.add_warning(ValidationError(
                        ...
                        {"attribute": attr_name, "expected_type": str(expected_type)},
                        severity="warning"
                    ))
                else:
                    ...
                    if not self._types_compatible(impl_type, expected_type):
                        result.add_warning(ValidationError(
                            ...
                            {"attribute": attr_name, "expected": str(expected_type), "actual": str(impl_type)},
                            severity="warning"
                        ))
        ...
        try:
            ...
            if len(impl_params) != len(protocol_params):
                result.add_warning(ValidationError(
                    ...
                    {"method": method_name, "expected": len(protocol_params), "actual": len(impl_params)},
                    severity="warning"
                ))
            
            # Check parameter names and types
            for i, (impl_param, protocol_param) in enumerate(zip(impl_params, protocol_params)):
                if impl_param.name != protocol_param.name:
                    result.add_warning(ValidationError(
                        ...
                        {"method": method_name, "parameter": i, "expected": protocol_param.name, "actual": impl_param.name},
                        severity="warning"
                    ))
