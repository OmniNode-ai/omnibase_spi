...
def test_artifact_container_validation():
    ...
    class TestArtifactContainer:
        ...
        def get_artifact_by_name(
            self, 
            name: str, 
            artifact_type: Optional[ContainerArtifactType] = None
        ) -> ProtocolArtifactInfo:
...
def test_validation_decorator():
    ...
    class DecoratedContainer:
        ...
        def get_artifacts_by_type(self, artifact_type): return []
        def get_artifact_by_name(self, name, artifact_type=None): raise ValueError("Not found")
        def has_artifact(self, name, artifact_type=None): return False
...
def test_error_scenarios():
    ...
    print(f"Result: {'✗ FAILED (Expected)' if not result.is_valid else '✓ Unexpected Pass'}")
    print(f"Errors: {len(result.errors)}, Warnings: {len(result.warnings)}")
