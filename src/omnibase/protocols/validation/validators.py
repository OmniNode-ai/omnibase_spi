...
from .protocol_validator import ProtocolValidator, ValidationResult, ValidationError
...
class ArtifactContainerValidator(ProtocolValidator):
    ...
    def validate_implementation(self, implementation: Any, protocol: Any = None) -> ValidationResult:
        """
    ...
    def _validate_artifact_container_business_rules(self, implementation: Any, result: ValidationResult) -> None:
        ...
        if hasattr(implementation, 'get_artifacts') and hasattr(implementation, 'get_status'):
            try:
    ...
    def _validate_container_status(self, status: Any, result: ValidationResult) -> None:
        ...
        required_attrs = ['status', 'message', 'artifact_count', 'valid_artifact_count', 
                         'invalid_artifact_count', 'wip_artifact_count', 'artifact_types_found']
        ...
        if hasattr(status, 'artifact_count') and hasattr(status, 'valid_artifact_count') and hasattr(status, 'invalid_artifact_count'):
            try:
    ...
    def _validate_artifact_consistency(self, artifacts: List[Any], status: Any, result: ValidationResult) -> None:
        """Validate consistency between artifacts list and status."""
        try:
            actual_count = len(artifacts) if artifacts else 0
            
            if hasattr(status, 'artifact_count') and status.artifact_count != actual_count:
                result.add_warning(ValidationError(
                    "artifact_count_mismatch",
                    f"Status reports {status.artifact_count} artifacts but get_artifacts() returns {actual_count}",
                    {"status_count": status.artifact_count, "actual_count": actual_count},
                    severity="warning"
                ))
            ...
            if hasattr(status, 'artifact_types_found') and artifacts:
                ...
                if artifact_types_in_list != status_types:
                    result.add_warning(ValidationError(
                        ...
                        {"status_types": list(status_types), "actual_types": list(artifact_types_in_list)},
                        severity="warning"
                    ))
    ...
    def _validate_artifact_type_filtering(self, implementation: Any, result: ValidationResult) -> None:
        """Validate artifact type filtering functionality."""
        try:
            # Test with a known artifact type
            artifact_types = ["nodes", "cli_tools", "runtimes", "adapters", "contracts", "packages"]
            
            for artifact_type in artifact_types[:2]:  # Test first two types only
                try:
                    filtered_artifacts = implementation.get_artifacts_by_type(artifact_type)
                    
                    # Verify all returned artifacts have the correct type
                    for artifact in filtered_artifacts:
                        if hasattr(artifact, 'artifact_type') and artifact.artifact_type != artifact_type:
                            result.add_error(ValidationError(
                                "artifact_type_filter_error",
                                f"get_artifacts_by_type('{artifact_type}') returned artifact with type '{artifact.artifact_type}'",
                                {"requested_type": artifact_type, "actual_type": artifact.artifact_type}
                            ))
...
class NodeRegistryValidator(ProtocolValidator):
    ...
    def _validate_node_registry_business_rules(self, implementation: Any, result: ValidationResult) -> None:
    ...
    def _validate_registry_initialization(self, implementation: Any, result: ValidationResult) -> None:
    ...
    def _validate_async_methods(self, implementation: Any, result: ValidationResult) -> None:
        """Validate that registry methods are properly async."""
        async_methods = [
            'register_node', 'unregister_node', 'update_node_health',
            'heartbeat', 'discover_nodes', 'get_node', 'get_nodes_by_group',
            'get_gateway_for_group', 'watch_node_changes', 'stop_watch'
        ]
    ...
    def _validate_discovery_functionality(self, implementation: Any, result: ValidationResult) -> None:
...
class HandlerDiscoveryValidator(ProtocolValidator):
    ...
    def _validate_handler_discovery_business_rules(self, implementation: Any, result: ValidationResult) -> None:
    ...
    def _validate_discovered_handlers(self, handlers: List[Any], result: ValidationResult) -> None:
    ...
    def _validate_handler_info(self, handler_info: Any, index: int, result: ValidationResult) -> None:
        """Validate individual handler info object."""
        required_attrs = ['node_class', 'name', 'source', 'priority', 'extensions', 'special_files', 'metadata']
            ...
            try:
                ...
                if not isinstance(extensions, list):
                    result.add_warning(ValidationError(
                        ...
                        {"index": index, "extensions_type": type(extensions).__name__},
                        severity="warning"
                    ))
            except Exception:
                pass
...
class ServiceRegistryValidator(ProtocolValidator):
    ...
    def validate_implementation(self, implementation: Any, protocol: Any = None) -> ValidationResult:
        ...
        if protocol:
            ...
            impl_name = implementation.__class__.__name__ if hasattr(implementation, '__class__') else str(type(implementation))
    ...
    def _validate_generic_registry_patterns(self, implementation: Any, result: ValidationResult) -> None:
        """Validate common registry patterns."""
        
        # Check for common registry methods
        common_registry_methods = [
            'register', 'unregister', 'get', 'list', 'find', 
            'discover', 'lookup', 'resolve'
        ]
        ...
        storage_indicators = [
            '_registry', '_storage', '_cache', '_backend', '_data',
            'registry', 'storage', 'cache', 'backend', 'data'
        ]
