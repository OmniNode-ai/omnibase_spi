...
def example_basic_validation():
    ...
    class GoodArtifactContainer:
        ...
        def get_artifact_by_name(self, name: str, artifact_type: Optional[ContainerArtifactType] = None) -> ProtocolArtifactInfo:
            raise NotImplementedError("Artifact not found")
        
        def has_artifact(self, name: str, artifact_type: Optional[ContainerArtifactType] = None) -> bool:
            return False
...
def example_validation_decorators():
    ...
    class DecoratedGoodContainer:
        ...
        def get_artifacts(self) -> List[ProtocolArtifactInfo]:
            return [self._mock_artifact("test1", "nodes"), self._mock_artifact("test2", "cli_tools")]
...
def example_specialized_validators():
    ...
    class TestContainer:
        ...
        def get_artifact_by_name(self, name: str, artifact_type: Optional[ContainerArtifactType] = None):
            for artifact in self.artifacts:
                if artifact.name == name and (artifact_type is None or artifact.artifact_type == artifact_type):
                    return artifact
            raise ValueError(f"Artifact {name} not found")
        
        def has_artifact(self, name: str, artifact_type: Optional[ContainerArtifactType] = None):
            try:
                self.get_artifact_by_name(name, artifact_type)
                return True
            except ValueError:
        ...
        def _create_artifact(self, name: str, artifact_type: str):
            class Artifact:
                def __init__(self, name: str, artifact_type: str):
                    self.name = name
                    self.artifact_type = artifact_type
                    self.version = type('Version', (), {'major': 1, 'minor': 0, 'patch': 0, '__str__': lambda s: '1.0.0'})()
...
def example_development_workflow():
    ...
    print(f"Missing methods: {len([e for e in result.errors if e.error_type == 'missing_method'])}")
